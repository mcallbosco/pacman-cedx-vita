...
so_hook hook_thumb(uintptr_t addr, uintptr_t dst) {
    ...
    if (addr & 2) {
        uint16_t nop = 0xbf00;
        h.orig_halfword = *(volatile uint16_t *)addr;
        h.thumb_unaligned = 1;
...
static int dynlib_cmp(const void *a, const void *b) {
    return strcmp(((const so_default_dynlib *)a)->symbol,
                  ((const so_default_dynlib *)b)->symbol);
}

static int dynlib_key_cmp(const void *key, const void *elem) {
    return strcmp((const char *)key, ((const so_default_dynlib *)elem)->symbol);
}

/* Sort default_dynlib in place so bsearch can replace the O(H) strcmp loop.
 * Cached by (pointer, count); repeat calls with the same array are no-ops. */
static void ensure_dynlib_sorted(so_default_dynlib *arr, int count) {
    static so_default_dynlib *sorted_arr = NULL;
    static int sorted_count = 0;
    if (arr == sorted_arr && count == sorted_count) return;
    qsort(arr, count, sizeof(so_default_dynlib), dynlib_cmp);
    sorted_arr = arr;
    sorted_count = count;
}
...
int so_resolve(so_module *mod, so_default_dynlib *default_dynlib, int size_default_dynlib, int default_dynlib_only) {
    uintptr_t val;
    int count = size_default_dynlib / sizeof(so_default_dynlib);
    ensure_dynlib_sorted(default_dynlib, count);
    for (int i = 0; i < mod->num_reldyn + mod->num_relplt; i++) {
        Elf32_Rel *rel = i < mod->num_reldyn ? &mod->reldyn[i] : &mod->relplt[i - mod->num_reldyn];
        Elf32_Sym *sym = &mod->dynsym[ELF32_R_SYM(rel->r_info)];
        uintptr_t *ptr = (uintptr_t *)(mod->text_base + rel->r_offset);
        ...
        switch (type) {
            ...
            case R_ARM_JUMP_SLOT:
            {
                if (sym->st_shndx == SHN_UNDEF) {
                    ...
                    so_default_dynlib *hit = bsearch(name, default_dynlib, count, sizeof(so_default_dynlib), dynlib_key_cmp);
                    if (hit) {
                        val = hit->func;
                        kuKernelCpuUnrestrictedMemcpy(ptr, &val, sizeof(uintptr_t));
                        resolved = 1;
    ...
    for (int i = 0; i < mod->num_reldyn + mod->num_relplt; i++) {
        ...
        switch (type) {
            ...
            case R_ARM_JUMP_SLOT:
            {
                if (sym->st_shndx == SHN_UNDEF) {
                    const char *name = mod->dynstr + sym->st_name;
                    if (bsearch(name, default_dynlib, count, sizeof(so_default_dynlib), dynlib_key_cmp)) {
