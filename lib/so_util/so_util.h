...
typedef struct {
    ...
    /* For thumb hooks whose target address is 2 mod 4: the LDR.W PC,[PC]
     * literal-pool load requires a 4-byte-aligned PC, so we NOP the 2 bytes
     * at (addr-2) and place the trampoline at addr. orig_halfword stores the
     * original 2 bytes so SO_CONTINUE can restore them before calling the
     * original function through thumb_addr. */
    uint16_t orig_halfword;
    uint8_t  thumb_unaligned;
} so_hook;
...
static inline void so_hook_unpatch(const so_hook *h) {
    kuKernelCpuUnrestrictedMemcpy((void *)h->addr, (void *)h->orig_instr, sizeof(h->orig_instr));
    if (h->thumb_unaligned) {
        kuKernelCpuUnrestrictedMemcpy((void *)(h->addr - 2), (void *)&h->orig_halfword, sizeof(h->orig_halfword));
        kuKernelFlushCaches((void *)(h->addr - 2), sizeof(h->orig_instr) + 2);
    } else {
        kuKernelFlushCaches((void *)h->addr, sizeof(h->orig_instr));
    }
}

static inline void so_hook_repatch(const so_hook *h) {
    kuKernelCpuUnrestrictedMemcpy((void *)h->addr, (void *)h->patch_instr, sizeof(h->patch_instr));
    if (h->thumb_unaligned) {
        uint16_t _so_nop = 0xbf00;
        kuKernelCpuUnrestrictedMemcpy((void *)(h->addr - 2), &_so_nop, sizeof(_so_nop));
        kuKernelFlushCaches((void *)(h->addr - 2), sizeof(h->patch_instr) + 2);
    } else {
